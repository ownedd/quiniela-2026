...
export default function DashboardPage() {
  ...
  const canQueryProtectedData = isLoaded && !!user && isAuthenticated && !convexLoading;
  const users = useQuery(api.users.leaderboard, canQueryProtectedData ? {} : "skip") as LeaderboardRow[] | undefined;
  const settings = useQuery(api.tournamentSettings.get, canQueryProtectedData ? {} : "skip");
  const teamsQuery = useQuery(api.teams.list, canQueryProtectedData ? {} : "skip");
  const playersQuery = useQuery(api.bonusPredictions.getPlayers, canQueryProtectedData ? {} : "skip");
  const matchesByGroup = useQuery(api.matches.byGroup, canQueryProtectedData ? {} : "skip") ?? {};
  const userPredictions = useQuery(api.predictions.getMine, canQueryProtectedData ? {} : "skip") ?? [];
      ...
      <section className="glass-card-gold p-4 sm:p-6">
        ...
        <div className="space-y-2 stagger-children">
          {users === undefined ? (
            ...
            displayedUsers.map((currentUser, index) => (
              <div
                key={currentUser._id}
              ...
              >
                ...
                >
                  {currentUser.image ? (
                    <Image src={currentUser.image} alt={currentUser.displayName ?? ""} width={40} height={40} className="object-cover w-full h-full" unoptimized />
                  ) : (
                ...
                <div className="flex-1 min-w-0">
                  <p className={`font-semibold text-sm truncate ${index === 0 ? "text-gold-light" : ""}`}>{currentUser.displayName ?? "Participante"}</p>
                </div>
                ...
                <div className="flex flex-col items-end gap-0.5 shrink-0 text-right">
                  ...
                  {(currentUser.bonusPoints ?? 0) > 0 ? (
                    <span className="text-[10px] text-gray-500 font-display tabular-nums">
                      incluye {currentUser.bonusPoints ?? 0} de especiales
                    </span>
                  ) : null}
                </div>
              </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-start gap-3">
            ...
            <p className="text-sm font-medium">
              ...
              <span className="text-gray-400">10 puntos. Si hay empate entre varios, acertar con cualquiera de ellos vale.</span>
            </p>
          </div>
          <div className="flex items-start gap-3">
            <CheckCircle2 className="w-5 h-5 text-green shrink-0 mt-0.5" />
            <p className="text-sm font-medium">
              ...
              <span className="text-gray-400">10 puntos. Empates entre equipos: cualquiera de los empatados cuenta.</span>
            </p>
          </div>
          <div className="flex items-start gap-3">
            <CheckCircle2 className="w-5 h-5 text-green shrink-0 mt-0.5" />
            <p className="text-sm font-medium">
              ...
              <span className="text-gray-400">10 puntos. Misma regla si hay empate en la mejor defensa.</span>
            </p>
          </div>
        </div>
      </section>
